...
### 4c. 検証の深さ (= 本リポジトリの中核責務)

このリポジトリの価値は「公式サイトより詳しい、裏取り済みの機能リスト・動作仕様」であることにある。CHANGELOG の 1 行を true/false で確認して終わらせない。各項目について次を自問し、答えられるまで掘る:

- **その機能・設定は実際にどう動くか**: 「追加された」で終わらせず、動かして挙動を見る。公式 docs に書いてあることが実際は動かない・違う挙動をする事例が過去に何度もあった (= 疑ってかかる)
- **plugin / hook / skill 系の変更**: 実際にサンプル plugin / hook / skill を書いて動かす。読むだけで済ませない
- **matcher の境界**: ワイルドカードはどこまでマッチするか (例: ハイフン付き識別子、`mcp__<server>__*` 等)
- **path の境界**: project 相対のみか、絶対パスも通るか、`~` や環境変数展開はされるか
- **新規 hook**: その hook で実際に何ができるか (block 可否、output の効果)、JSON input/output の実際のフィールド構成 (公式 docs に無いフィールドが乗っていないか)
- **frontmatter / 設定値の細かい挙動**: 型・省略時デフォルト・複数指定時の扱い (上書きか追記か) を実際に試す

「該当箇所を見つけて 1 行転記した」は検証ではない。実際に動かして確認できて初めて `[実機検証済]` を名乗れる。
...
## 8. kawaz への報告 (= メンテパスの最終成果物、省略不可)

push で終わりではない。journal はリポジトリの記録、この節は **kawaz 本人への報告**。目的が違うので内容も分ける。

「作業手順を何ステップやったか」の報告 (issue 起票した、agent を N 並列で回した等) はここでは求められていない。kawaz が知りたいのは以下の 3 点:

1. **前回検証時からどれだけ進んだか**: バージョン範囲 (vA.B.C → vX.Y.Z)、期間
2. **各変更が実際どう動くか、検証の確からしさ**: `[実機検証済]` / `[spec]` / `[未検証]` の内訳と、特に「検証したら公式記述と違った」「実際は使えなかった」という反証があれば最優先で
3. **その中で重要・面白い差分の解説**: 機械的な列挙でなく、なぜそれが重要か・何が意外だったかを噛み砕いて説明する

新機能が無かった/地味な patch だった回は無理に膨らませない。反証や新発見が多い回は厚めに書く。
